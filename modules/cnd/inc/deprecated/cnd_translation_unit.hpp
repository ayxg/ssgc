...
#include "cnd_cldev_logger.hpp"
#include "cnd_cli_predefval.hpp"
// clang-format on

namespace cnd::compiler {
struct TranslationFragment {};
struct TranslationUnit {
  cldev_util::Logger* cli_stdio{&cldev_util::gStdLog()};          ///> Streams for CLI out/err/in at compile time.
  eVerbosityLevel cli_verbosity_level{eVerbosityLevel::kNormal};  ///> Verbosity level for CLI output.
  Bool is_overwrite_allowed{false};                               ///> Overwrite disabled by default.
  Vec<cli::PredefinedValue> predefs{};                            ///> Predefined values from the CLI.
  ...
  TranslationFragment Fragment() { return {}; }

};


struct TranslationPartition {};
