...
DEF_GRAMMAR_ELEMENT(KwEnum, "enum");
...
#define KEYWORD_GRAMMAR_ELEMENTS                                               \
  KwDef, KwFn, KwClass, KwMain, KwImport, KwNamespace, KwUse, KwLib, KwDll,    \
      KwEnum,KwIf, KwElif, KwElse, KwCxif, KwCxelif, KwCxelse, KwSwitch, KwCase,      \
...
static constexpr inline auto kAllKeywords = std::array{
    ...
    kKwUse,    kKwLib,    kKwDll,kKwEnum,    kKwIf,      kKwElif,     kKwElse,
...
static constexpr inline auto kAllDirectives = std::array{
...
constexpr std::string_view eAstEnumStr(eAst t);
...
struct TkTrait {
  ...
  static constexpr std::string_view enum_str = eTkEnumStr(TkType);
  static constexpr std::string_view literal = eTkLiteral(TkType);
...
template <eTk... TkType>
using TraitsOf = std::tuple<TkTrait<TkType>...>;
