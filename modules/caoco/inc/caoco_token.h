...
class Tk {
 public:
  // Modifiers
  constexpr void SetFile(std::size_t file) { file_ = file; }
  constexpr void SetLine(std::size_t line) { line_ = line; }
  constexpr void SetCol(std::size_t col) { col_ = col; }
  constexpr void SetSourceIndexBeg(std::size_t idx) { source_index_beg_ = idx; }
  constexpr void SetSourceIndexEnd(std::size_t idx) { source_index_end_ = idx; }
  // Properties
  ...
  constexpr std::size_t SourceIndexBeg() const noexcept {
    return source_index_beg_;
  }
  constexpr std::size_t SourceIndexEnd() const noexcept {
    return source_index_end_;
  }
 ...
 public:
  ...
  constexpr Tk(eTk type, std::vector<char>::const_iterator beg,
               ...
               std::tuple<std::size_t, std::size_t> src_idx);
  constexpr Tk(eTk type, std::vector<char>::const_iterator beg,
               std::vector<char>::const_iterator end, std::size_t line,
               std::size_t col) noexcept;
  constexpr Tk(eTk type, std::string literal);
  constexpr Tk(eTk type, std::string literal, std::size_t line,
               std::size_t col);
  constexpr Tk(eTk type, std::string literal,
               std::tuple<std::size_t, std::size_t> src_idx);
 ...
 private:
  ...
  std::size_t source_index_beg_{0};
  /// 1 past the end char in the source code.
  std::size_t source_index_end_{0};
};

constexpr Tk::Tk(eTk type, std::vector<char>::const_iterator beg,
                 std::vector<char>::const_iterator end) noexcept
...
constexpr Tk::Tk(eTk type, std::string literal)
    : type_(type), line_(0), col_(0), literal_(literal) {}

constexpr Tk::Tk(eTk type, std::string literal, std::size_t line,
                 std::size_t col)
