...
Target Properties:
  MINITEST_TARGET_PROP_TEST_HEADERS

Global Variables:
  USE_MINITEST_AUXILLARY_DIR
...
#[=[
  @global USE_MINITEST_AUXILLARY_DIR
  @brief [READ-ONLY] '${CMAKE_FILES_DIRECTORY}/UseMinitest' minitest generated code and scan results dir.
#]=]
set(USE_MINITEST_AUXILLARY_DIR "${CMAKE_FILES_DIRECTORY}/UseMinitest")

#[=[
  @global MINITEST_TARGET_PROP_TEST_HEADERS
  @brief [READ-ONLY] Specifies header files to scan for test cases for a given minitest target.
#]=]
define_property(
  TARGET 
...
function(minitest_add_executable)
  ...
  set(generated_main_file "${USE_MINITEST_AUXILLARY_DIR}/generated_main/${arg_NAME}.cpp")
  set(include_lines "")
  ...
  # USE_MINITEST_TARGET_MINITEST_LIBRARY : Determine which minitest library target to use.
  if(TARGET minitest_library) # local target
    set(USE_MINITEST_TARGET_MINITEST_LIBRARY minitest_library)
  elseif(TARGET minitest_box::minitest_library) # exported target
    set(USE_MINITEST_TARGET_MINITEST_LIBRARY minitest_box::minitest_library)
  else() # fall back to find_library
    find_library(
      USE_MINITEST_TARGET_MINITEST_LIBRARY
      NAMES minitest_library minitest_box::minitest_library
      NO_CACHE
    )
    if(USE_MINITEST_TARGET_MINITEST_LIBRARY STREQUAL "USE_MINITEST_TARGET_MINITEST_LIBRARY-NOTFOUND")
      message(FATAL_ERROR 
        "[minitest_add_executable] Unable to find minitest_library or minitest_box::minitest_library."
      )
    endif()
  endif()
  ...
  target_link_libraries(${arg_NAME} ${USE_MINITEST_TARGET_MINITEST_LIBRARY} ${arg_LINK_LIBS})
...
function(z_minitest_suite_run_scan)
  ...
  set(scan_result_dir "${USE_MINITEST_AUXILLARY_DIR}/scan_results")

  # Run test target in scan mode post build, using --scan command.
  set(temp_scan_result_file 
    "${scan_result_dir}/minitest_scan_result_for_${arg_PREFIX}_${arg_TARGET}.txt")
  set(this_test_scan_result_file "${scan_result_dir}/${arg_PREFIX}_${arg_TARGET}.txt")
  ...
  add_custom_command(
    ...
    WORKING_DIRECTORY ${scan_result_dir}
  )
  add_custom_command(
    TARGET ${arg_TARGET} POST_BUILD
    COMMAND 
