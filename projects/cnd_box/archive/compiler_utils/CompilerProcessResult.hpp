...
using ClMsgBuffer = cldev::clmsg::ClMsgBuffer;
...
//  using Ex<ValueT, ErrorT>::expected;  // use the constructor provided by std::expected.
//  using Ex<ValueT, ErrorT>::operator=;
...
template <class T>
using ClRes = Ex<T, ClMsgBuffer>;
using ClFail = Unex<ClMsgBuffer>;

template <class T>
using CompilerProcessResult = Ex<T, ClMsgBuffer>;
