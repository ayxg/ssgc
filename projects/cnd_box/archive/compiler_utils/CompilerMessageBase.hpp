...
///
/// -[01/13/2025] The ClMsg structure should be tiny(8 bytes) to minimize stack space. When used as
/// an alternative for variant expected , optional: ClMsg should not cause an increase in the union
/// size. The expectation is that IF a ClMsg is returned then the program has already encountered an
/// invalid or error state, or we are printing verbose diagnostics in which case unpacking speed
/// does not matter. As such we can use many layers of indirection to dynamically allocate a message
/// upon construction. The overall underlying data structure of ClMsg currently is:
///    'std::unique_ptr<std::vector<std::variant<ClMsgNode,std::vector<ClMsgNode>>>>'
/// This is all wrapped up and separated into the ClMsgNode, ClMsgChain, ClMsgUnion and ClMsg
/// classes.
///
...
/// on the enum entry. By static casting the enum to ClMsgCodeIntT or ClMsgCategoryIntT, we turn
/// this into a pseudo-'compile time virtual method'. Intellisense should be able to display the
/// required arguments to create the valid data for this message.
///
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
static_assert(sizeof(ClMsgCategoryIntT) == 1 &&
              "cnd::cldev::clmsg::ClMsgCategoryIntT must be 1 byte.");
static_assert(sizeof(ClMsgParameterIntT) == 2 &&
              "cnd::cldev::clmsg::ClMsgParameterIntT must be 2 bytes.");
...
constexpr ClMsgId GetClMsgIdOf(eClWarning e) {
  return ClMsgId{static_cast<ClMsgCodeIntT>(e),
                 static_cast<ClMsgCategoryIntT>(GetClWarningCategory(e)),
                 static_cast<ClMsgCategoryIntT>(GetClMsgType(e))};
};

constexpr ClMsgId GetClMsgIdOf(eClGuide e) {
  return ClMsgId{static_cast<ClMsgCodeIntT>(e),
                 static_cast<ClMsgCategoryIntT>(GetClGuideCategory(e)),
                 static_cast<ClMsgCategoryIntT>(GetClMsgType(e))};
};

constexpr ClMsgId GetClMsgIdOf(eClDiagnostic e) {
  return ClMsgId{static_cast<ClMsgCodeIntT>(e),
                 static_cast<ClMsgCategoryIntT>(GetClDiagnosticCategory(e)),
                 static_cast<ClMsgCategoryIntT>(GetClMsgType(e))};
};
...
using ClMsgDataUnionT =
    ccapi::Var<I64, UI64, Str>;                 // Union of allowed compiler message data types.
...
constexpr Str FormatClMsg(ClMsgId id,
                          const ClMsgDataBufferT& data) noexcept;  // MUST be forward declared here.

/////////////////////////////////////////////
/* Compiler message structure definitions. */
/////////////////////////////////////////////

/// The fundamental compiler message type. !DO NOT construct directly, use MakeClMsg method to
/// create a message.
///
...
struct ClMsgUnion : ClMsgUnionUnderlyingT {
  ...
  constexpr void PushBack(const ClMsgUnion& e) {
    if (IsSingle()) TransformToChain();

    if (std::holds_alternative<ClMsgNode>(e))
      GetChain().PushBack(get<ClMsgNode>(e));
    else
      for (const auto& msg : get<ClMsgChain>(e).messages) {
        GetChain().PushBack(msg);
      }
  };

  constexpr void PushBack(ClMsgUnion&& e) {
    if (IsSingle()) TransformToChain();
...
class ClMsgBuffer {
 public:
  explicit constexpr ClMsgBuffer() noexcept
      : messages_(make_unique<Vec<ClMsgUnion>>(Vec<ClMsgUnion>{})) {}
  ...
  ClMsgBuffer(const ClMsgBuffer& other)
      : messages_(make_unique<Vec<ClMsgUnion>>(*other.messages_)) {}
  ...
  constexpr ClMsgId GetLastMessageId() const noexcept {
    return messages_->back().GetLastMessageId();
  }

  constexpr void chain(const ClMsgUnion& msg) {
    if (messages_->size() > 0)
      messages_->back().PushBack(msg);
    else
      push(msg);
  }
  ...
  constexpr void push(const ClMsgUnion& msg) { messages_->push_back(msg); }
...
//////////////////////////////////////////////////////////////////////
//
// This is required or else the vtable method in FormatClErr cannot lookup the method defs. Note the
// use of CND_MM_CONST_ID_TAG to get the class enum symbol from the applied enum. Since every format
// method signature is the same, we can pre-declare them using macros.
//
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Define the manual formatting dispatch method FormatClMsg(ClMsgId id, const ClMsgDataBufferT&
 * data) */
/////////////////////////////////////////////////////////////////////////////////////////////////////////////

// The 'manual vtable' for FormatClErr method. Dispatches to the specialized template
// implementations. !warning DO NOT call directly. Called only by 'ClMsgNode.Format()'.
...
constexpr ClMsgDataBufferT ConvertCppSourceLocationToClMsgData(
...
///
/// Used to define compiler message creation method for a specific eClMsg id based on the enum
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Define example make and formatting methods for the first entry of eClErr, eClWarning, eClGuide
 * and eClDiagnostic */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
...
#if !defined(CND_COMPILER_IMPL_DISABLE_STATIC_UNIT_TESTS) && \
    CND_STATIC_UNIT_TEST_COMPILER_MESSAGE_BASE

// Assert we can access the last error code in a constexpr context.
static_assert(MakeClMsg<eClErr::kNoError>().GetLastMessageCode() ==
              to_underlying(eClErr::kNoError));
static_assert(MakeClMsg<eClWarning::kNoWarning>().GetLastMessageCode() ==
              to_underlying(eClWarning::kNoWarning));
static_assert(MakeClMsg<eClGuide::kNoGuide>().GetLastMessageCode() ==
              to_underlying(eClGuide::kNoGuide));
...
static_assert(
    FormatClMsg<eClWarning::kNoWarning>(MakeClMsg<eClWarning::kNoWarning>().GetSingle().data) ==
    "[False Positive Warning]");
static_assert(FormatClMsg<eClGuide::kNoGuide>(MakeClMsg<eClGuide::kNoGuide>().GetSingle().data) ==
              "[False Positive Guide]");
static_assert(FormatClMsg<eClDiagnostic::kNoDiagnostic>(
                  MakeClMsg<eClDiagnostic::kNoDiagnostic>().GetSingle().data) ==
