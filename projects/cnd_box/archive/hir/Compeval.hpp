...
//  Namespace global{.parent = nullptr, .name = kGlobalNamespaceName};
//
...
//         auto rhs_lit = ident_val_ir.ReduceConsteval();
//         if (not rhs_lit) return Expected<PrimaryExpr>::Failure(rhs_lit.Error());
//         auto result = CtAddOp(lhs_lit.Extract(), rhs_lit.Extract());
//         return Expected<PrimaryExpr>::Success(result);
//       }
...
//     if (stmt.TypeIs(eAst::kVariableDeclaration) || stmt.TypeIs(eAst::kVariableDefinition)) {
...
//     } else if (stmt.TypeIs(eAst::kMethodDeclaration) || stmt.TypeIs(eAst::kMethodDefinition)) {
...
//     } else if (stmt.TypeIs(eAst::kKwUse)) {
...
//     } else if (stmt.TypeIs(eAst::kClassDeclaration)) {
//       auto use_def_ir = UsingDecl::FromAst(stmt, program, program.global_ns, eval_order);
//       if (not use_def_ir) return Expected<Program>::Failure(use_def_ir.Error());
//     } else if (stmt.TypeIs(eAst::kKwNamespace)) {
//       auto use_def_ir = UsingDecl::FromAst(stmt, program, program.global_ns, eval_order);
//       if (not use_def_ir) return Expected<Program>::Failure(use_def_ir.Error());
//     }
//   }
...
//                                                 size_t& eval_order) {
//   if (ast.TypeIsnt(eAst::kKwNamespace)) {
