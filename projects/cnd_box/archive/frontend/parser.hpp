...
#define DEBUG_FAIL(msg)                                                              \
  CompilerProcessFailure(                                                            \
      cldev::clmsg::MakeClMsg<corevals::diagnostic::eClErr::kCompilerDevDebugError>( \
...
struct DeclParser {
  ...
  static constexpr LLPrsResT ParseNamedDeclaration(TkCursorT c, eTk keyword, eAst decl_ast_type,
                                                   eAst def_ast_type,
    ...
    // Expect keyword
    if (c.TypeIsnt(keyword)) return DEBUG_FAIL("Expected declaration keyword.");
    c.Advance();
    ...
    if (c.TypeIs(eTk::kColon)) {
      ...
      return LLParserResult(
          c, Ast(decl_ast_type, decl_begin, c.Iter(), nullptr, {mod_node, ident_node, def_node}));
    } else if (c.TypeIs(eTk::kSemicolon)) {
      c.Advance();
      return LLParserResult(
          c, Ast(decl_ast_type, decl_begin, c.Iter(), nullptr, {mod_node, ident_node}));
    } else {
  ...
  static constexpr LLPrsResT ParseUnnamedDeclaration(TkCursorT c, eTk keyword, eAst decl_ast_type,
                                                     LLPrsResT (*parse_def)(TkCursorT)) noexcept {
    using enum eTk;
    using namespace detail;
    ...
    if (c.TypeIs(eTk::kColon)) {
      ...
      c.Advance(def_result.value().head);
      return LLParserResult(
          c, Ast(decl_ast_type, decl_begin, c.Iter(), {mod_node, def_result.value().ast}));
    } else {
...
constexpr LLPrsResT ParseAccess(TkCursorT c) noexcept;
...
constexpr ScopePrsResT FindStatement(eTk open, eTk close, TkConstIterT begin,
                                     TkConstIterT end) noexcept;
constexpr ScopePrsResT FindOpenStatement(eTk close, TkConstIterT begin, TkConstIterT end) noexcept;
constexpr ScopePrsResT FindOpenStatement(Vec<eTk> close, TkConstIterT begin,
                                         TkConstIterT end) noexcept;
...
constexpr SepScopePrsResT FindSeperatedParen(TkConstIterT begin, TkConstIterT end,
                                             eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedParen(TkCursorT crsr, eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedParen(TkScopeT ls, eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedBrace(TkConstIterT begin, TkConstIterT end,
                                             eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedBrace(TkCursorT crsr, eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedBrace(TkScopeT ls, eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedBracket(TkConstIterT begin, TkConstIterT end,
                                               eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedBracket(TkCursorT crsr, eTk separator) noexcept;
constexpr SepScopePrsResT FindSeperatedBracket(const TkScopeT& ls, eTk separator) noexcept;
...
constexpr LLPrsResT ParseListingArguments(TkCursorT c) noexcept {
  using enum eTk;
  if (c.TypeIs(eTk::kLBrace)) return DEBUG_FAIL("ImplExpectedToken");
...
constexpr LLPrsResT ParseFunctionalStmt(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kSemicolon)) return DEBUG_FAIL("Empty statement in code.");
...
constexpr LLPrsResT ParseIfDecl(TkCursorT c) noexcept {
  ...
  LAMBDA xParseIfElifElse = [&c](eAst ast_type) -> LLPrsResT {
    if (c.TypeIs(eTk::kKwIf) || c.TypeIs(eTk::kKwElif)) {
  ...
  if (c.TypeIs(eTk::kSemicolon)) return LLParserResult(c.Advance(), move(ifelifelse_stmt));

  while (c.TypeIs(eTk::kKwElif)) {
...
constexpr LLPrsResT ParseVariableDecl(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kColon)) {
    ...
    c.Advance(def_result.value().head);
    Ast def_node =
        Ast(eAst::kVariableDefinition, def_begin, c.Iter(), nullptr, {def_result.value().ast});

    return LLParserResult(c, Ast(eAst::kVariableDeclaration, decl_begin, c.Iter(), nullptr,
...
constexpr LLPrsResT ParseMethodDecl(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kColon)) {
    ...
    c.Advance();
    return LLParserResult(c, Ast(eAst::kMethodDeclaration, decl_begin, c.Iter(), nullptr,
...
constexpr LLPrsResT ParseImportDecl(TkCursorT c) noexcept {
  ...
  // Expecting an identifier.
  if (not c.TypeIs(eTk::kIdent)) return DEBUG_FAIL("Expected <ident>.");
  Ast ident_node = c;
  c.Advance();
  ...
  return LLParserResult(c,
                        Ast(eAst::kImportDeclaration, stmt_begin, c.Iter(), nullptr, {ident_node}));
}
...
constexpr LLPrsResT ParseLibDecl(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kColon)) {
    ...
    c.Advance(def_result.value().head);
    return LLParserResult(c, Ast(eAst::kLibraryDeclaration, decl_begin, c.Iter(),
                                 {move(mod_node), move(def_result.value().ast)}));
  }
  ...
  if (c.TypeIs(eTk::kSemicolon)) {
    c.Advance();
    return LLParserResult(
        c, Ast(eAst::kLibraryDeclaration, decl_begin, c.Iter(), {mod_node, ident_node}));
  }
  ...
  c.Advance(def_result.value().head);
  return LLParserResult(
      c, Ast(eAst::kLibraryDeclaration, decl_begin, c.Iter(), {mod_node, ident_node, def_node}));
}
...
constexpr LLPrsResT ParseMainDecl(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kCommercialAt)) {
    ...
    c.Advance();
    return LLParserResult(c,
                          Ast(eAst::kMainDeclaration, decl_begin, c.Iter(), {sig_node, def_node}));
  }
}
...
constexpr LLPrsResT ParseMethodParameters(TkCursorT c) noexcept {
  ...
  constexpr LAMBDA xFindCommercialAt = [](TkConstIterT beg, TkConstIterT end) -> bool {
    TkCursorT c(beg, end);
  ...
  if (c.TypeIsnt(kLParen)) return DEBUG_FAIL("Expected <(>.");
  ...
  // - If there is an @ then it is a typed argument -> Expect possible mods, followed a
  // primaryPreIdentifier.
  ...
  for (const auto& scope : arg_scopes.value()) {
    ...
    if (scope.ContainedBegin()->TypeIs(kIdent)) {
      param_list.PushBack(Ast(eAst::kMethodParameter, param_begin, c.Iter(),
                              Ast(eAst::kModifiers, Ast(eAst::kNONE)), Ast(eAst::kKwAny),
...
constexpr LLPrsResT ParseMethodSignature(TkCursorT c) noexcept {
  using enum eAst;
  using namespace detail;
  constexpr LAMBDA xMake1ParamSigAst = [](Ast ret, Ast param) {
    return Ast(kMethodSignature, Ast(kMethodParameterList, Ast(kMethodParameter, param)),
               Ast(kMethodReturnType, ret));
  };

  constexpr LAMBDA xMakeSigAst = [](Ast ret_type_ast, Ast param_list_ast) {
  ...
  // GreaterThan after identifier -> Some sort of void arg with a return.
    ...
    if (c.TypeIs(eTk::kColon))
      return LLParserResult(c.Advance(), xMake1ParamSigAst(kKwAny, kMethodVoid));
    ...
    return LLParserResult(
        c, xMakeSigAst(return_type_result.value().head,
  ...
  // Open Paren After Identifier -> Method with arguments.
    ...
    if (c.TypeIs(eTk::kSemicolon)) {
      return LLParserResult(c, xMakeSigAst(Ast(kMethodReturnType, Ast(kMethodVoid)),
                                           method_params_result.value().ast));
    }
    ...
    if (c.TypeIs(eTk::kColon)) {
      ...
      if (c.TypeIs(eTk::kColon)) {
        c.Advance();
        return LLParserResult(
            c, xMakeSigAst(Ast(kMethodReturnType, Ast(kKwAny)), method_params_result.value().ast));
      }
      ...
      c.Advance(return_type_result.value().head);
      return LLParserResult(
          c, xMakeSigAst(return_type_result.value().ast, method_params_result.value().ast));
    } else {
...
constexpr LLPrsResT ParseIncludeStmt(TkCursorT c) noexcept {
  ...
  if (c.TypeIsnt(kKwInclude)) return DEBUG_FAIL("Expected <keyword-include>.");
  c.Advance();

  // String literal -> local include.
  if (c.TypeIs(eTk::kLitCstr)) {
    Ast ident_node = c;
    ident_node.type = eAst::kIQCharSeuquence;
    c.Advance();
    if (c.TypeIsnt(kSemicolon)) return DEBUG_FAIL("Expected <semicolon>.");
    c.Advance();
    return LLParserResult(
        c, Ast(eAst::kIncludeLocalStmt, stmt_begin, c.Iter(), nullptr, {ident_node}));
  }
  // Less than -> system include.
  else if (c.TypeIs(eTk::kLt)) {
    auto ia_char_sequence_begin = c.Iter();
    c.Advance();
    while (c.TypeIsnt(kGt)) {
      c.Advance();
    }

    if (c.TypeIsnt(kGt)) return DEBUG_FAIL("Unclosed system inclusion.");
    c.Advance();

    Ast ia_char_sequence = Ast(eAst::kIACharSeuquence, ia_char_sequence_begin, c.Iter());

    if (c.TypeIsnt(kSemicolon)) return DEBUG_FAIL("Expected <semicolon>.");
    c.Advance();
    return LLParserResult(
        c, Ast(eAst::kIncludeSystemStmt, stmt_begin, c.Iter(), nullptr, {ia_char_sequence}));
  }
...
constexpr LLPrsResT ParseProcDef(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kSemicolon)) return DEBUG_FAIL("Empty process definition.");

  // Process block
  if (c.TypeIs(eTk::kLBrace)) {
    c.Advance();

    if (c.TypeIs(eTk::kRBrace))  // Empty Proc def
    {
      c.Advance();
      return LLParserResult{c, process_node};
    } else {
      while (!c.AtEnd() && c.TypeIsnt(kRBrace)) {
...
constexpr LLPrsResT ParseProcDecl(TkCursorT c) noexcept {
  ...
  if (c.TypeIsnt(kCommercialAt)) return DEBUG_FAIL("Expected '@' symbol after 'proc' keyword.");
  c.Advance();
  if (c.TypeIsnt(kIdent)) return DEBUG_FAIL("Expected identifier after '@' symbol.");
  Ast ident_node{c};
  c.Advance();
  ...
  if (c.TypeIs(eTk::kSemicolon)) {
    c.Advance();
    return LLParserResult(
        c, Ast(eAst::kProcessDeclaration, decl_begin, c.Iter(), {mod_node, ident_node}));
  }

  if (c.TypeIsnt(kColon)) return DEBUG_FAIL("Expected ':' symbol followed by process definition.");
  c.Advance();

  auto def_result = ParseProcDef(c);
  if (!def_result) return def_result;
  c.Advance(def_result->head);
  return LLParserResult(c, Ast(eAst::kProcessDeclaration, decl_begin, c.Iter(),
...
constexpr LLPrsResT ParseUsingDecl(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kCommercialAt)) {
    c.Advance();
    if (c.TypeIsnt(kIdent)) return DEBUG_FAIL("Expected <ident>.");
    Ast alias = Ast(c);
    c.Advance();
    if (c.TypeIsnt(kColon)) return DEBUG_FAIL("Expected <:>.");
    c.Advance();
    ...
    c.Advance(val_expr.value().head);
    return LLParserResult(
        c, {eAst::kTypeAlias, stmt_begin, c.Iter(), nullptr, {alias, val_expr->ast}});
  }

  // - lib-> Library Namespace Inclusion.
  else if (c.TypeIs(eTk::kKwLib)) {
    c.Advance();
    auto val_expr = ParsePrimaryStatement(c);
    if (!val_expr) return val_expr;
    c.Advance(val_expr.value().head);
  ...
  else if (c.TypeIs(eTk::kKwNamespace)) {
    c.Advance();
    auto val_expr = ParsePrimaryStatement(c);
    if (!val_expr) return val_expr;
    c.Advance(val_expr->head);
    return LLParserResult(
        c, Ast(eAst::kNamespaceInclusion, stmt_begin, c.Iter(), nullptr, {val_expr->ast}));
  }
  ...
  // The primary expr assumed to resolve to an object or function name. Validate at a later stage.
    ...
    return LLParserResult(c, Ast(eAst::kNamespaceObjectInclusion, stmt_begin, c.Iter(), nullptr,
                                 {primary_result->ast}));
...
constexpr LLPrsResT ParseNamespaceDecl(TkCursorT c) noexcept {
  if (c.TypeIsnt(eTk::kKwNamespace)) return DEBUG_FAIL("Expected 'namespace' keyword.");

  c.Advance();
  if (c.TypeIsnt(eTk::kCommercialAt))
    return DEBUG_FAIL("Expected '@' symbol after 'namespace' keyword.");
  c.Advance();
  if (c.TypeIsnt(eTk::kIdent)) return DEBUG_FAIL("Expected <identifier>.");
  Ast node{eAst::kNamespaceDecl};
  node.children.push_back(Ast(c));

  c.Advance();
  if (c.TypeIs(eTk::kSemicolon)) return LLParserResult(c.Advance(), node);
  if (c.TypeIsnt(eTk::kLBrace)) return DEBUG_FAIL("Expected <{>.");

  while (!c.AtEnd() || c.TypeIsnt(eTk::kRBrace)) {
    if (c.IsDirectiveFirstSet()) {
      auto directive_desc = ParseDirectiveDesc(c);
      if (!directive_desc) return directive_desc;
      c.Advance(directive_desc.value().head);
      node.PushBack(move(directive_desc.value().ast));
    } else
...
constexpr LLPrsResT ParseSyntax(TkCursorT c) noexcept {
  ...
  while (!c.AtEnd()) {
    if (c.IsDirectiveFirstSet()) {
      ...
      if (!directive_desc) return directive_desc;
      c.Advance(directive_desc.value().head);
      program_node.PushBack(move(directive_desc.value().ast));
...
constexpr LLPrsResT ParseEnumDecl(TkCursorT c) noexcept {
  ...
  if (c.TypeIs(eTk::kColon)) {
    ...
    return LLParserResult(
        c, Ast(eAst::kEnumDeclaration, decl_begin, c.Iter(), mod_node, ident_node, def_node));
  } else if (c.TypeIs(eTk::kSemicolon)) {
    c.Advance();
    return LLParserResult(c,
                          Ast(eAst::kEnumDeclaration, decl_begin, c.Iter(), mod_node, ident_node));
  } else {
    return DEBUG_FAIL("Expected <;> or <:>.");
  }
};
...
constexpr LLPrsResT ParseEnumDef(TkCursorT c) noexcept {
  ...
  while (c.TypeIsnt(kLBrace)) {
    ...
    if (c.IsPrimary()) {
      ...
      assoc_node.PushBack(Ast{eAst::kKwAny, c.Iter(), c.Iter()});
    }

    // <commercial_at> <identifier>?
    if (c.TypeIs(eTk::kCommercialAt) && !is_unnamed_associative_enum) {
      c.Advance();
    ...
    if (c.TypeIs(eTk::kColon)) {
      ...
      return LLParserResult(c.Advance(), node);
    } else {
      return DEBUG_FAIL("Expected <;> or <:>.");
    }
  }
...
constexpr LLPrsResT ParseEnumBlock(TkCursorT c) noexcept {
  ...
  if (c.TypeIsnt(eTk::kLBrace))  // Require opening brace.
    return DEBUG_FAIL("Expected <{>.");
  c.Advance();

  while (c.TypeIsnt(eTk::kRBrace)) {
    // EnumEntry
    if (c.TypeIs(eTk::kCommercialAt)) {
      c.Advance();
      if (c.TypeIsnt(eTk::kIdent)) return DEBUG_FAIL("Expected <identifier>.");
      ...
      while (c.TypeIsnt(eTk::kSemicolon)) {
        if (c.TypeIsnt(eTk::kColon)) return DEBUG_FAIL("Expected <:>.");
        c.Advance();
        
        if (!c.IsPrimary()) return DEBUG_FAIL("Expected <primary-expr>.");
        LLPrsResT expr_result = ParsePrimaryExpr(c);
        if (!expr_result) return LLPrsResT::unexpected_type(expr_result.error());
    ...
    // EnumCategory
      ...
      while (c.TypeIsnt(eTk::kColon)) {
        if (c.TypeIsnt(eTk::kCommercialAt)) return DEBUG_FAIL("Expected <@>.");
        c.Advance();
        if (c.TypeIsnt(eTk::kIdent)) return DEBUG_FAIL("Expected <identifier>.");
...
constexpr ScopePrsResT FindBrace(TkCursorT c) noexcept {
  if (c.AtEnd()) return DEBUG_FAIL("OutOfBounds");  // Out of bounds begin passed to method...
  if (c.TypeIsnt(eTk::kLBrace))
    return DEBUG_FAIL("Expected opening scope.");  // No open token to start with.
  auto scope_begin = c.Iter();
  // c.Advance();
  if (c.AtEnd())
    return DEBUG_FAIL("kParserOpeningScopeAtEof.");  // End right after open, cannot be closed.
  if (c.TypeIs(eTk::kRBrace))
...
constexpr ScopePrsResT FindBracket(TkCursorT c) noexcept {
  if (c.AtEnd()) return DEBUG_FAIL("OutOfBounds");  // Out of bounds begin passed to method...
  if (c.TypeIsnt(eTk::kLBracket))
    return DEBUG_FAIL("Expected opening scope.");  // No open token to start with.
  auto scope_begin = c.Iter();
  c.Advance();
  if (c.AtEnd())
    return DEBUG_FAIL("kParserOpeningScopeAtEof.");  // End right after open, cannot be closed.
  if (c.TypeIs(eTk::kRBracket))
    return TkScopeT{true, scope_begin, c.Advance().Iter()};  // Empty paren scope '()'
  return FindScopeImpl(c);
}  // end find_paren
...
constexpr LLPrsResT ParseGenericBinaryLeftAssociative(
  ...
  if (!IsTkPrimarySpecifier(c.Type()))
  ...
  // Note the accum vector holds (operand -> operator) pairs to keep track of the specific operator
  // type.
...
constexpr LLPrsResT ParseAssignment(TkCursorT c) noexcept {
  ...
  if (accum_rhs.size() == 1)
    return LLParserResult{c, move(accum_rhs.back())};  // No reduction necessary.
...
constexpr LLPrsResT ParseComparison(TkCursorT c) noexcept {
  ...
  return detail::ParseGenericBinaryLeftAssociative(
      c,
      [](const auto& a) {
        return a.TypeIs(eTk::kGt) || a.TypeIs(eTk::kGte) || a.TypeIs(eTk::kLt) ||
               a.TypeIs(eTk::kLte);
      },
...
constexpr LLPrsResT ParsePrefix(TkCursorT c) noexcept {
  ...
  while (c.IsPrefixOperator()) {
    if (c.TypeIs(eTk::kAdd)) {
      accum_op.emplace_back(c);
      accum_op.back().type = eAst::kUnaryPlus;
    } else if (c.TypeIs(eTk::kSub)) {
      accum_op.emplace_back(c);
      accum_op.back().type = eAst::kUnaryMinus;
    } else
      accum_op.emplace_back(c);
    c.Advance();
  }
...
constexpr LLPrsResT ParsePostfixAccess(TkCursorT c) noexcept {
  using std::move;
  using std::next;
  using std::ranges::subrange;
...
constexpr LLPrsResT ParsePostfix(TkCursorT c) noexcept {
  ...
  while (c.IsPostfixOperator() || c.IsOpeningScope() || c.TypeIs(eTk::kPeriod)) {
    ...
    // Binary member access.
      ...
      accum_op.emplace_back(eAst::kMemberAccess, c.Iter(), access_rhs->ast.src_end,
                            move(access_rhs->ast));
...
constexpr SepScopePrsResT FindSeperatedScopeImpl(TkConstIterT begin, TkConstIterT end,
                                                 eTk separator, eTk scope_type) noexcept {
  Vec<TkScopeT> scopes;
  auto closing_scope = [=] {
    switch (scope_type) {
      case eTk::kLParen:
