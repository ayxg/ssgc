...
constexpr bool IsTkPrimarySpecifier(eTk t) noexcept {
  return IsTkAnOperand(t) || IsTkAPrefixOperator(t) || t == eTk::kLParen || IsTkLScope(t) || t == eTk::kDoubleColon;
}
...
#define CND_MM_LOCAL_CASE(n, lt) else if (cxx::StrEq(kw, lt)) return eTk::k##n
  ...
  // Pragmatic
  CND_MM_LOCAL_CASE(KwInclude, "include");
  CND_MM_LOCAL_CASE(KwPragma, "pragma");
  // Declarative
