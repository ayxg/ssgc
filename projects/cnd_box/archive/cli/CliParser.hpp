...
class Parser {
 ...
 public:
    ...
    for (; arg_it < end; arg_it++) {
      ...
      switch (flags_.at(flag_idx).interp) {
        ...
        case eFlagInterp::kCmd: { // This is in case a command is passed prefixed with a '--'.
          if (HasCommand()) {
            command_ = flags_.at(flag_idx).id;
            return arg_it + 1;  // Rest args
          } else                // Unparsed args are denied by default.
            return unexpected{format("Unexpected argument: '{}'.", *arg_it)};
        } break;
  ...
  std::string GenerateHelpText() {
    ...
    if (HasCommand()) {
      help_text += "Commands:\n";
      for (const auto& command_idx : lookup_cmd_) {
        help_text += std::format("    {}{}{}:\n{}\n", command_idx.first,
                                 ...
                                 flags_.at(command_idx.second).long_name[0] != '\0'
                                     ...
                                     : "",
                                 IndentParagraph(flags_.at(command_idx.second).desc, 8));
      }
    }
    ...
    if (HasPositional()) {
      ...
      for (const auto& pos_idx : lookup_pos_) {
        help_text +=
            std::format("    {}:\n{}\n", flags_.at(pos_idx).long_name, IndentParagraph(flags_.at(pos_idx).desc, 8));
      }
    }
    ...
    for (const auto& flag : flags_) {
      if (flag.interp == eFlagInterp::kCmd || flag.interp == eFlagInterp::kPositional) continue;
      ...
      help_text += std::format("    {}:\n{}\n", flag_repr, IndentParagraph(flag.desc, 8));
    }
    return help_text;
  }
  std::string IndentParagraph(const std::string& text, size_t indent_width) {
