...
TEST(UtParserPrimaryExprOperands, WholeNumber) {
  TestParsingMethod("1", ParseOperand, Sast{kLitI32, "1"});
}
...
TEST(UtParserPrimaryExprTopDown, BinaryTermThenSum) {
  TestParsingMethod("1 * 2 + 3", ParsePrimaryExpr,
                    Sast(kAdd, "1*2+3", Sast(kMul, "1*2", Sast(kLitI32, "1"), Sast(kLitI32, "2")),
                         Sast(kLitI32, "3")));
}

TEST(UtParserPrimaryExprTopDown, BinarySumIsLeftAssociative) {
  TestParsingMethod("1 + 2 - 3", ParsePrimaryExpr,
                    Sast(kSub, "1+2-3", Sast(kAdd, "1+2", Sast(kLitI32, "1"), Sast(kLitI32, "2")),
                         Sast(kLitI32, "3")));
}
...
TEST(UtParserPrimaryExprTopDown, ParenIsResolvedFirst) {
  TestParsingMethod(
      ...
      Sast(kMul, "(1+2)*3",
           Sast(kSubexpression, "(1+2)", Sast(kAdd, "1+2", Sast(kLitI32, "1"), Sast(kLitI32, "2"))),
           Sast(kLitI32, "3")));
}
...
TEST(UtParserPrimaryExprTopDown, PrefixUnaryThenBinary) {
  TestParsingMethod("!1+2", ParsePrimaryExpr,
                    Sast(kAdd, "!1+2", Sast(kNot, "!1", Sast(kLitI32, "1")), Sast(kLitI32, "2")));
}
...
TEST(UtParserPrimaryExprTopDown, PrefixUnaryAfterBinary) {
  TestParsingMethod("1+!2", ParsePrimaryExpr,
                    Sast(kAdd, "1+!2", Sast(kLitI32, "1"), Sast(kNot, "!2", Sast(kLitI32, "2"))));
}
