...
#include "trtools/compositor.hpp"
...
int CliMain(int argc, char* argv[], char* envp[]) {
  ...
  switch (cli_args.mode) {
    ...
    case eDriverFlag::kModeComp: {
      CompilerProcessResult<TranslationInput> comp_params_res = driver::HandleCliArgsCompMode(cli_args);
      if (!comp_params_res) return log.PrintErrForward(comp_params_res.error());
      CompilerProcessResult<TranslationOutput> comp_res = trtools::Compose(comp_params_res.value());
      if (!comp_res)
