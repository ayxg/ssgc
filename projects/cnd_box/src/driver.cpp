...
ClResult<ParsedFlagOffset> ParseSingleArgFlag(ArgsBufferIter args_it, ArgsBufferIter args_end, Size flag_offset) {
  ...
  if (thisarg_it != thisarg_end) {
    ...
    return ParsedFlagOffset{.args = {{thisarg_it, thisval_it}},
                            .next_arg = args_it+1,
                            .arg_offset = 0};
  }
  ...
  return ParsedFlagOffset{.args = {{nextarg_it, nextarg_end}}, .next_arg = args_it + 1,
                          .arg_offset = 0};
};
...
ClResult<TranslationInput> HandleCliArgsCompMode(CommandLineArguments& inargs) {
  using enum eDriverFlag;
  assert(inargs.ContainsFlag(kModeComp) && "cnd::cli::HandleCliArgsCompMode expects composition mode flag.");
  ...
  auto params = TranslationInput{&cldev::util::gStdLog()};
...
ClResult<int> HandleCliArgsPostCompMode(TranslationInput& params, TranslationOutput& output) {
  return cldev::util::gStdLog().PrintErrForward(
