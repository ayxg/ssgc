...
/// @{
#ifndef HEADER_GUARD_SSG_MTA_MTA_HPP
#define HEADER_GUARD_SSG_MTA_MTA_HPP
// Includes:
...
/// @brief Contiguous Enumeration
template <class EnumT, EnumT FirstV, EnumT LastV,
          std::invocable<EnumT> auto... AssociatedValues>
  requires std::is_scoped_enum<EnumT>::value
struct contig_enum {
  using under_t = std::underlying_type_t<EnumT>;
  using enum_t = EnumT;
  static constexpr inline enum_t first_val = FirstV;
  static constexpr inline enum_t last_val = LastV;
  static constexpr inline under_t size =
      static_cast<under_t>(LastV) - static_cast<under_t>(FirstV) + 1;
  static constexpr inline size_t num_associated_values =
      sizeof...(AssociatedValues);
  using results_of_assoc_invoke =
      std::tuple<decltype(AssociatedValues(std::declval<enum_t>()))...>;
  template<class T>
  using Array = std::array<T, size>;
  
  static constexpr enum_t to_enum(under_t e) { return static_cast<enum_t>(e); }
  static constexpr under_t to_under(enum_t e) {
    return static_cast<under_t>(e);
  }
  ...
  template <size_t N>
  static constexpr auto& assoc(enum_t idx) {
    return std::get<N>(associated_values[to_under(idx)]);
  }
  ...
  constexpr under_t under() { return static_cast<under_t>(val); }

  constexpr contig_enum begin() { return *this; }
  constexpr contig_enum end() {
    return contig_enum{static_cast<EnumT>(to_under(last_val) + 1)};
  }
...
#endif HEADER_GUARD_SSG_MTA_MTA_HPP
