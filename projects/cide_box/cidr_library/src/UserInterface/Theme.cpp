...
// clang-format off
#include "CIDR/UserInterface/Theme.hpp"
...
backend::JsonObj Theme::ToJson(const Theme& obj) noexcept {
  backend::JsonObj j{};
  if (obj.style) {
    // Settings
...
Theme Theme::FromJson(const backend::JsonObj& obj) noexcept {
...
void Theme::Apply(StyleT& curr_style) noexcept {
  ...
  if (!font_path.empty() && std::filesystem::exists(font_path)) {
    ...
    auto font_exists = std::find_if(io.Fonts->Fonts.begin(), io.Fonts->Fonts.end(), [&](const auto& font) {
      return font->ConfigData->Name == font_name;
    });
